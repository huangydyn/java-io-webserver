...
import com.huangydyn.reactor.basic.BasicHandler;
 ...
 * <p>
 * <b>注意这里的 AnotherReactor 是 MultiReactor 的内部类</b>
 *
 * @see AnotherReactor
 */
public class MultiReactor {
    ...
    private AnotherReactor mainReactor;

    // 从 Reactors，用于处理 I/O，可使用 BasicHandler 和  MultithreadHandler 两种处理方式
    private AnotherReactor[] subReactors = new AnotherReactor[POOL_SIZE - 1];
    int next = 0;

    public MultiReactor(int port) {
        try {
            this.port = port;
            mainReactor = new AnotherReactor();

            for (int i = 0; i < subReactors.length; i++) {
                subReactors[i] = new AnotherReactor();
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        public synchronized void run() {
            try {
                ...
                if (sc != null) {
                    ...
                    AnotherReactor subReactor = subReactors[next];
...
//					new MultithreadHandler(subSel, sc);
    ...
    static class AnotherReactor implements Runnable {
        private ConcurrentLinkedQueue<BasicHandler> events = new ConcurrentLinkedQueue<>();
        final Selector selector;

        public AnotherReactor() throws IOException {
